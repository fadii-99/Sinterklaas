...
class MediaProcessor:
    ...
    def create_pip_clip(self):
        ...
        if self.image_paths:
            try:
                ...
                # Define the rotation matrix for 14 degrees clockwise
                rotation_matrix = cv2.getRotationMatrix2D(center, -14, 1.0)  # -14 for clockwise rotation

                # Perform the rotation
                rotated_img = cv2.warpAffine(resized_img, rotation_matrix, (w, h), borderMode=cv2.BORDER_REFLECT)

                # Convert the rotated image (OpenCV format) to a PIL Image
                rotated_img_pil = Image.fromarray(cv2.cvtColor(rotated_img, cv2.COLOR_BGR2RGBA))

                # Save the rotated image to use with ImageClip
                resized_image_path = "resized_image.png"
                rotated_img_pil.save(resized_image_path, "PNG")

                # Create the PIP clip with the rotated image
                pip_clip = (
                    ImageClip(resized_image_path, transparent=True)
                    .set_duration(11.05)
                    .set_fps(24)
                )
    ...
    def compress_video(self, video_path):
        """Compress video to reduce size below 20MB."""
        compressed_path = f"static/generated_videos/compressed_{os.path.basename(video_path)}"
        clip = VideoFileClip(video_path)

        
        # Reduce resolution and bitrate to compress
        ...
        final_path = video_path.replace("compressed_", "")
        os.rename(compressed_path, final_path)

        # Verify the size after compression
        if os.path.getsize(final_path) > 20 * 1024 * 1024:
            raise Exception("Unable to compress the video below 20MB.")
...
def generate_video(purchase_id, purchase_date, purchase_email, files, final_video_dir, connection, generated_audio):
    ...
    processor.save_final_video(final_video_path, audio_path=generated_audio, fv2_path="static/endingpart.mp4", email=purchase_email)
...
def send_receipt_email(email: str, payment_id: str, video_name: str, amount_value: str, currency: str, reciever, send_date):
    ...
    <body style="background-color: #ffffff; font-family: Arial, sans-serif; color: #333; margin: 0; padding: 20px;">
        <div style="max-width: 600px; margin: auto; border: 1px solid #c00; border-radius: 5px; overflow: hidden;">
            <div style="background-color: #c00; padding: 20px; color: #ffffff;">
                <h1 style="margin: 0;">SintMagie Betalingsbewijs</h1>
            </div>
            <div style="padding: 20px;">
                <p style="font-size: 16px;">Bedankt voor uw betaling!</p>
                <hr style="border: 1px solid #c00; margin: 20px 0;">
                <p style="font-size: 14px;"><strong>Betalings-ID:</strong> {payment_id}</p>
                <p style="font-size: 14px;"><strong>Videonaam:</strong> {video_name}</p>
                <p style="font-size: 14px;"><strong>Bedrag:</strong> {amount_value} {currency}</p>
                <p style="font-size: 14px;"><strong>Ontvanger:</strong> {reciever}</p>
                <p style="font-size: 14px;"><strong>Verzenddatum:</strong> {send_date}</p>
                <p style="font-size: 14px;">Uw steun betekent veel voor ons!</p>
            </div>
            <div style="background-color: #f7f7f7; padding: 10px; text-align: center;">
                <p style="font-size: 12px; color: #888; margin: 0;">Wij wensen u een vrolijke SintMagie viering!</p>
            </div>
        </div>
    </body>
</html>
