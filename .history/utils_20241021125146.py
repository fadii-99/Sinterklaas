...
class MediaProcessor:
    ...
    def create_padded_image_clip(self, img_path, target_height):
        img = Image.open(img_path)
        width, height = img.size
        ...
        # Create the resized image clip
        img_clip = ImageClip(img_path).resize(height=target_height).set_duration(6)
        
        # Calculate the left and right margins to center the clip
        left_margin = (self.max_width - new_width) // 2
        img_clip = img_clip.margin(
            left=left_margin,
            right=left_margin,
            color=(0, 0, 0)  # Black margins
        )
        
        return img_clip

    def create_padded_video_clip(self, video_path, target_height):
        ...
        # Resize the video to match the height of clip_1
    ...
    def process_media(self):
        ...
        # Load clip_1 and use its height for scaling user media
        clip_1 = self.load_static_clip('clip_1.mp4')
        target_height = clip_1.h  # Get the height of clip_1
        final_clips.append(clip_1)

        # Pick a single media file (image or video) and scale it accordingly
        if self.image_paths:
            media_clip = self.create_padded_image_clip(self.image_paths[0], target_height)
        elif self.video_paths:
            media_clip = self.create_padded_video_clip(self.video_paths[0], target_height)
            media_clip = media_clip.set_duration(6)  # Ensure it's exactly 6 seconds

        final_clips.append(media_clip)

        # Add the remaining clips
