...
def get_pending_video(connection):
    ...
    try:
        query = """
            SELECT id, voice_path, created_at, user_email
...
def generate_video(
    purchase_id, purchase_date, purchase_email, files, final_video_dir: str, connection, generated_audio: str
):
    print("\n\nGenerating video")
    ...
    # Generate a video from image files if no video files exist
    if not video_paths:
        print("No video files found. Creating video from images.")
        video_clip = ImageSequenceClip(image_paths, fps=1)  # 1 frame per second
    else:
        # Concatenate multiple video files if present
        clips = [VideoFileClip(video) for video in video_paths]
        video_clip = concatenate_videoclips(clips)

    # Load the generated audio and set it as the background audio
    if generated_audio:
...
def generate_voice( email, date, name, age, hobby):
    print('\nGenerating voice\n')
    try:
