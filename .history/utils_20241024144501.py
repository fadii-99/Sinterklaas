...
class MediaProcessor:
    ...
    def create_end_clip(self):
        """Create an ending black screen with 'The End' text."""
        end_text = TextClip(
            "The End", fontsize=80, color='white', font='Amiri-Bold', size=(1280, 720)
        ).set_duration(5).set_fps(24)

        # Black background clip
        black_background = ImageClip(color=(0, 0, 0), size=(1280, 720)).set_duration(5).set_fps(24)

        # Overlay the text on the black background
        end_clip = CompositeVideoClip([black_background, end_text.set_position('center')])
    ...
    def verify_audio_path(audio_path):
