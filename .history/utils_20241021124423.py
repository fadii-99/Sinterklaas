...
class MediaProcessor:
    def __init__(self, image_paths, video_paths, target_width=1280, target_height=720):
    ...
    def load_static_clip(self, clip_name):
        print('load_static_clip')
        """Load predefined static clips from the 'static/clips' directory."""
        clip_path = f'static/clips/{clip_name}'
        return VideoFileClip(clip_path)

    def resize_image_with_opencv(self, img_path):
        print('resize_image_with_opencv')
        """Resize and crop the image to match target dimensions using OpenCV."""
        img = cv2.imread(img_path)  # Load the image using OpenCV
        resized_img = cv2.resize(img, (self.target_width, self.target_height), interpolation=cv2.INTER_AREA)
        return ImageClip(resized_img).set_duration(6)

    def create_padded_video_clip(self, video_path):
        print('create_padded_video_clip')
        """Resize and crop video to the target dimensions."""
        video_clip = VideoFileClip(video_path)
        video_clip = video_clip.resize((self.target_width, self.target_height))
        return video_clip

    def process_media(self):
        """Process media in the desired sequence."""
        final_clips = []

        # Load and append clip_1
        final_clips.append(self.load_static_clip('clip_1.mp4'))

        # Process images or videos if available
        if self.image_paths:
            final_clips.append(self.resize_image_with_opencv(self.image_paths[0]))
        elif self.video_paths:
            media_clip = self.create_padded_video_clip(self.video_paths[0])
            media_clip = media_clip.set_duration(6)  # Ensure 6 seconds duration
            final_clips.append(media_clip)

        # Append other static clips in sequence
        final_clips.append(self.load_static_clip('clip_3.mp4'))
        final_clips.append(self.load_static_clip('clip_4.mp4'))

        # Concatenate all clips with proper FPS handling
        if len(final_clips) == 0:
            raise ValueError("No media files to process.")

        # Ensure all clips are processed with the same FPS
        try:
            fps = final_clips[0].fps  # Use FPS from the first clip
        except AttributeError:
            fps = 24  # Set a default FPS if not found

        final_clip = concatenate_videoclips(final_clips, method="compose")
        final_clip = final_clip.set_fps(fps)  # Ensure consistent FPS
    ...
    def save_final_video(self, output_path, audio_path=None, fps=24):
        ...
        print('save_final_video')
        final_clip = self.process_media()

        # Use FPS from the first video if available; otherwise, fall back to default FPS
        if self.video_paths:
            video_clip = VideoFileClip(self.video_paths[0])
            fps = video_clip.fps  # Extract FPS from the first video clip

        if audio_path:
            audio_clip = VideoFileClip(audio_path).audio
            final_clip = final_clip.set_audio(audio_clip)

        # Write the final video with the determined FPS
...
#         # random.shuffle(self.media_paths)  # Optional random shuffle, if desired
...
#     def load_static_clip(self, clip_name):
#         clip_path = os.path.join('static/clips', clip_name)
#         return VideoFileClip(clip_path)

#     def create_padded_image_clip(self, img_path):
#         img = Image.open(img_path)
#         width, height = img.size
#         img_clip = ImageClip(img_path).set_duration(6)  # Set duration to 6 seconds
...
#         # Pick a single media file (image or video) for 6 seconds
#         if self.image_paths:
#             final_clips.append(self.create_padded_image_clip(self.image_paths[0]))
#         elif self.video_paths:
#             media_clip = self.create_padded_video_clip(self.video_paths[0])
...
def generate_video(purchase_id, purchase_date, purchase_email, files, final_video_dir, connection, generated_audio):
    ...
    if not image_paths and not video_paths:
        raise ValueError("No valid media files found for video generation.")
    ...
    # Process media and generate video
    ...
    # Insert/update video record in the database
    cursor = connection.cursor()
    insert_query = """
...
def generate_voice( email, date, name, age, hobby,purchase_id, connection):
    print('\nGenerating voice\n')
    try:
        text = f"Ohhh, hello there! How wonderful that I get to record a very special message just for you, {name}! You know, I’ve just looked into my big book, and I’ve read so much about you. But first, I want to tell you how happy I am that it’s almost Sinterklaas Eve! What an exciting time, right? With pepernoten, gifts, and so much fun! Oh, and look who’s here with me... Piet wanted to say hello to you too! Well, {age}, I heard you’re already [age] years old! You’re growing up so fast! And do you know what else I know? You love {hobby}, is that right? That’s so much fun!"
