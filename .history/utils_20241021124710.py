...
class MediaProcessor:
    ...
    def process_media(self):
        ...
        # Load and append static clip_1
        final_clips.append(self.load_static_clip('clip_1.mp4'))

        # Handle media (either image or video)
        if self.image_paths:
            # Create image clip and set FPS
            image_clip = self.resize_image_with_opencv(self.image_paths[0])
            image_clip = image_clip.set_duration(6).set_fps(24)  # Set 6 seconds duration and FPS
            final_clips.append(image_clip)
        elif self.video_paths:
            # Handle video clip and set duration
        ...
        # Ensure valid clips are available
        if len(final_clips) == 0:
            raise ValueError("No valid clips to process.")

        # Handle FPS consistency
        fps =  24

        # Concatenate all clips and set consistent FPS
