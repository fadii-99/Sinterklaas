...
from PIL import Image, ImageClip, ImageSequenceClip, Resampling
...
class MediaProcessor:
    def __init__(self, image_paths, video_paths, target_width=1280, target_height=720):
        ...
        self.target_width = target_width  # Desired width for all media
        self.target_height = target_height  # Desired height for all media
    ...
    def resize_and_crop_image(self, img_path):
        """Resize and crop the image to match target dimensions."""
        img = Image.open(img_path)
        img = img.resize((self.target_width, self.target_height), Resampling.LANCZOS)  # Use LANCZOS instead of ANTIALIAS
        return ImageClip(np.array(img)).set_duration(6)

    def create_padded_image_clip(self, img_path):
        """Resize and center-crop image to the target dimensions."""
        img_clip = self.resize_and_crop_image(img_path)
