...
class MediaProcessor:
    ...
    def save_final_video(self, output_path="final_output_random_duration.mp4", fps=24):
...
def save_final_video(db, purchase_id: int, final_video_path: str):
    query = """
        INSERT INTO final_videos (purchase_id, final_video_path, send_status)
        VALUES (%s, %s, %s)
    """
    cursor = db.cursor()
    cursor.execute(query, (purchase_id, final_video_path,'completed'))
    db.commit()
    cursor.close()
...
def generate_video(purchase_id: int, files, final_video_dir: str, db):
    ...
    #     WHERE purchase_id = %s
    # """
    # cursor.execute(update_query, (final_video_path, purchase_id))
    # db.commit()
    # cursor.close()
