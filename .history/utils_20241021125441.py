...
class MediaProcessor:
    ...
    def create_padded_image_clip(self, img_path, target_height, target_width):
        img = Image.open(img_path)
        width, height = img.size

        # Calculate the new dimensions to fit the height of `clip_1` while keeping the aspect ratio
        ...
        # Convert resized image to an array for ImageClip
        img_array = np.array(resized_img)

        # Create ImageClip from the resized image
        img_clip = ImageClip(img_array).set_duration(6)

        # Add black margins if the new width is less than the target width
        if new_width < target_width:
            left_margin = (target_width - new_width) // 2
            img_clip = img_clip.margin(left=left_margin, right=left_margin, color=(0, 0, 0))

        return img_clip

    def create_padded_video_clip(self, video_path, target_height, target_width):
        video_clip = VideoFileClip(video_path)
        width, height = video_clip.size

        # Calculate the new dimensions to fit the height of `clip_1` while maintaining aspect ratio
        new_width = int((target_height / height) * width)
        new_height = target_height
        ...
        if new_width < target_width:
            left_margin = (target_width - new_width) // 2
            video_clip = video_clip.margin(left=left_margin, right=left_margin, color=(0, 0, 0))
    ...
    def process_media(self):
        ...
        # Load clip_1 and get its dimensions
        clip_1 = self.load_static_clip('clip_1.mp4')
        target_height = clip_1.h
        target_width = clip_1.w
        final_clips.append(clip_1)

        # Process user-provided media (either image or video)
        if self.image_paths:
            media_clip = self.create_padded_image_clip(self.image_paths[0], target_height, target_width)
        elif self.video_paths:
            media_clip = self.create_padded_video_clip(self.video_paths[0], target_height, target_width)
            media_clip = media_clip.set_duration(6)  # Ensure 6-second duration

        final_clips.append(media_clip)

        # Add clip_2 and other remaining clips
