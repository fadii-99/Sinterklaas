...
async def clone_voice(files: list[UploadFile] = File(...)):
    try:
        ...
        # Clone voice with Eleven Labs API
        url = "https://api.elevenlabs.io/v1/voices/clone"
        headers = {
            "xi-api-key": XI_API_KEY
        }
        files_data = [("files", (os.path.basename(path), open(path, "rb"))) for path in sample_paths]
        payload = {"name": "CustomVoice", "description": "A cloned voice from uploaded samples"}
        ...
        raise HTTPException(status_code=500, detail=f"Error cloning voice: {e}")
