...
async def purchase_video(    user_email: str = Form(...),    receiver_phone: str = Form(...),    send_date: str = Form(...),    video_name: str = Form(...),    voice_path: str = Form(...),    amount: str = Form(...),    files: List[UploadFile] = File(...),    db: Session = Depends(get_db)
):
    try:
        ...
        cursor.execute(insert_purchase_query, (
            user_email,
            receiver_phone,  # Ensure the list is correctly joined
            send_date,
            video_name,
            'pending',
            voice_file_path
        ...
        cursor.execute("""
            SELECT created_at FROM video_purchases WHERE id = %s
        """, (purchase_id,))
        created_at = cursor.fetchone()[0]  # Get the timestamp

        # Format the date for folder creation
        folder_date = created_at.strftime('%Y-%m-%d')  # Retrieve the last inserted ID
