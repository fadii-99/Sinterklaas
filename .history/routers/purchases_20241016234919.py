...
import uuid
from schemas import VideoPurchaseSchema, promptVoice
...
async def start_background_task(text, task_id):
    await generate_voice(text, task_id)

@router.post("/generate-voice/")
async def generate_voice_endpoint(data: promptVoice, background_tasks: BackgroundTasks):
    try:
        task_id = str(uuid.uuid4())
        task_status[task_id] = "In Progress"

        asyncio.create_task(start_background_task(data.prompt, task_id))
