...
async def purchase_video(
    ...
    try:
        # Path to the existing voice file in static/generated_audios
        ...
        # Insert purchase data into video_purchases table
        ...
        cursor.execute(insert_purchase_query, (
            purchase.user_email,
            ','.join(purchase.receiver_phone),  # Store multiple phones as a comma-separated string
            purchase.send_date,
            purchase.video_name,
            'pending',
            voice_file_path
        ))
        purchase_id = cursor.fetchone()[0]  # Get the generated purchase ID

        # Insert image and video paths into a separate table
