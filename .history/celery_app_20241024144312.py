...
#         # generated_audio = 'static/generated_audios/fawdmuhammad14@gmail.com_2024-10-24 13:44:50.mp3'
...
def process_pending_videos():
    ...
    try:
        ...
        # Generate voice and ensure the file is generated
        print("\nGenerating Voice\n")
        generated_audio = generate_voice(
            purchase_email, purchase_date, name, age, hobby, purchase_id, connection
        )
        ...
        # Update the purchase status to 'in-progress'
        update_purchase_status_to_in_progress(connection, purchase_id)
        print("Purchase status updated to 'in-progress'.")

        # Retrieve associated files for the video
        files = get_files_for_video(connection, purchase_id)
        print(f"Retrieved files: {files}")

        # Generate the final video
        final_video_dir = os.path.abspath("static/generated_videos")
        print(f"Generating video in directory: {final_video_dir}")

        final_video_path = generate_video(
            purchase_id, purchase_date, purchase_email, files,
            final_video_dir, connection, generated_audio
        )
