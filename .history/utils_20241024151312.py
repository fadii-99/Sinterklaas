...
class MediaProcessor:
    def __init__(self, image_paths, video_paths, pip_position=(500, 300)):
        ...
        random.shuffle(self.image_paths)  # Optional shuffle
    ...
    def create_pip_clip(self):
        """Create the Picture-in-Picture (PIP) clip."""
        if self.image_paths:
            ...
            pip_clip = ImageClip(rotated_image_path, transparent=True).set_duration(11.3).set_fps(24)
    ...
    def create_end_clip(self):
        """Create an ending clip with a black background and 'The End' text."""
        # Create a black screen with ColorClip
        black_background = ColorClip(size=(1280, 720), color=(0, 0, 0)).set_duration(5).set_fps(24)

        # Create the 'The End' text clip
        end_text = TextClip(
            "The End", fontsize=100, color='white', font='Amiri-Bold'
        )

        # Calculate the center position for the text
        text_width, text_height = end_text.size
        x_center = (1280 - text_width) // 2
        y_center = (720 - text_height) // 2

        # Set the static position at the center
        end_text = end_text.set_position((x_center, y_center)).set_duration(5).set_fps(24)

        # Overlay the text on the black background
        end_clip = CompositeVideoClip([black_background, end_text])
    ...
    def concatenate_final_video(self, final_clip, fv2_path, audio_duration):
        ...
        if remaining_duration > 0:
            fv2_clip = fv2_clip.subclip(0, remaining_duration)

            # Concatenate the main clip with the trimmed `fv2_clip`
            final_video = concatenate_videoclips([final_clip, fv2_clip])
        else:
            final_video = final_clip

        # Add the ending clip
        end_clip = self.create_end_clip()
    ...
    def save_final_video(self, output_path, audio_path, fv2_path, fps=24):
        ...
        """Save the final video with the correct duration."""
        audio_clip = AudioFileClip(audio_path)
        audio_duration = audio_clip.duration

        final_clip = self.process_pip_video()

        # Ensure the video duration matches the audio length by concatenating fv2.mp4
...
def generate_video(purchase_id, purchase_date, purchase_email, files, final_video_dir, connection, generated_audio):
    ...
    processor = MediaProcessor(image_paths=image_paths, video_paths=['static/fv1.mp4'], pip_position=pip_position)
    processor.save_final_video(final_video_path, audio_path=generated_audio, fv2_path="static/fv2.mp4")
...
def generate_voice(email, date, name, age, hobby, purchase_id, connection):
    ...
    try:
        # Dutch translation of your text
        text = (
            f"Ohhh, hallo daar! Wat geweldig dat ik een heel speciaal bericht "
            f"voor jou mag opnemen, {name}! Weet je, ik heb net in mijn grote boek gekeken, "
            f"en ik heb zoveel over jou gelezen. Maar eerst wil ik je vertellen hoe blij ik ben "
            f"dat het bijna Sinterklaasavond is! Wat een spannende tijd, toch? Met pepernoten, cadeautjes, en zoveel plezier! "
            f"Nou,<break time='1s'/> {name}, ik heb gehoord dat je al {age} jaar oud bent! "
            f"Je groeit zo snel op! En weet je wat ik nog meer weet? Je houdt van {hobby}, klopt dat? Dat is zo leuk!"
        )
