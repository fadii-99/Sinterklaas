...
def generate_voice( email, date, name, age, hobby,purchase_id, connection):
    print('\nGenerating voice\n')
    try:
        text = f"Ohhh, hello there! How wonderful that I get to record a very special message just for you, {name}! You know, I’ve just looked into my big book, and I’ve read so much about you. But first, I want to tell you how happy I am that it’s almost Sinterklaas Eve! What an exciting time, right? With pepernoten, gifts, and so much fun! Well, {name} I heard you’re already {age}, years old! You’re growing up so fast! And do you know what else I know? You love {hobby}, is that right? That’s so much fun!"
